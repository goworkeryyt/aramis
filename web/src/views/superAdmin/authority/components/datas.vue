<template>
  <div>
    <div class="clearflex" style="margin:10px 0 30px">
      <el-button
        class="fl-right submit"
        @click="authDataEnter"
      >确 定</el-button>
      <el-button
        class="fl-left default-btn"
        @click="all"
      >全选</el-button>
      <el-button
        class="fl-left default-btn"
...
export default {
  ...
  created() {
    ...
    this.row.dataAuthorityId &&
      this.row.dataAuthorityId.forEach((item) => {
        const obj =
          this.authoritys &&
          this.authoritys.filter((au) => au.authorityId === item.authorityId) &&
          this.authoritys.filter(
  ...
  methods: {
    ...
    self() {
      this.dataAuthorityId = this.authoritys.filter(
        (item) => item.authorityId === this.row.authorityId
      )
      this.$emit('changeRow', 'dataAuthorityId', this.dataAuthorityId)
      this.needConfirm = true
    },
